# -*- coding: utf-8 -*-
"""
Created on Sun Dec 29 09:40:10 2019
...
dict_iso = {"Afghanistan" : {'Alpha-2': 'AF', 'Alpha-3': 'AFG', 'Numeric code': '004', 'Link to ISO 3166-2': 'ISO 3166-2:AF', 'Independent': 'Yes'},
...
cursor = conn.cursor()
cursor.execute("SELECT Country FROM Academy")
records = cursor.fetchall()
all_countries_list = []
for item in records:
    stre = convertTuple(item) 
    all_countries_list.append(stre)
...
t = tuple(all_countries_list)
cursor.execute("SELECT * FROM "+database_name.replace('.sqlite', '')+" WHERE Title IN {}".format(t))
records = cursor.fetchall()
countr_dict = {}
# Dictionary for all th countries
for items in records:
    occur_norm = [i / j for i, j in zip(eval(items[1]), aver_coauth_numb_EAGE[5:])]
    countr_dict[items[0]] = occur_norm
#    print(sum(eval(items[1])))

df = pd.DataFrame()

for key, value in countr_dict.items():
    if key == 'Unknown academia':
        iso_alpha = 'ATA'
        iso_num = '010'
        publ_num_eage = sum(value)
    else:
        iso_alpha = dict_iso[key]['Alpha-3']   
        iso_num  = dict_iso[key]['Numeric code']  
        publ_num_eage = sum(value)
    ...
    df = df.append({'country' : key, 
                      'iso_alpha' : iso_alpha, 
                      'iso_num' : iso_num,
                      'publ_num_eage' : publ_num_eage } , ignore_index=True) 
    
print(df)
    
##    df = df.append({'country' : 'Republic of South Africa' , 
#                      'iso_alpha' : 'ZAF', 
#                      'iso_num' : 710,
...
#                     hover_name="country", # column added to hover information
                     size="publ_num_eage",            # size of markers
                     size_max=30,
...
fig.write_image('K:\\python\\sql\\images\\fig2.svg')  


#fig.savefig('K:\\python\\sql\\images\\.png', dpi=300)
fig.show()